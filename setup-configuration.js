...
function quickSetup() {
  ...
  const SPREADSHEET_ID = '1aLKEEw7Nx0O1DbZjnXnhOeDjcLRloLN0Y8K2SscZKIc'; // Updated to new sheet
...
// testConfiguration();
/**
 * Helper function to update the Spreadsheet ID
 * Run this to switch to a new Google Sheet
 * @param {string} newId - The new Spreadsheet ID
 */
function updateSpreadsheetId(newId) {
  if (!newId) {
    console.log('❌ Please provide a valid Spreadsheet ID');
    return;
  }

  const props = PropertiesService.getScriptProperties();
  props.setProperty('IMPACT_SPREADSHEET_ID', newId);

  console.log('✅ Updated Spreadsheet ID to: ' + newId);
  console.log('You can now run the main script.');
}

/**
 * Wrapper to switch to the specific new sheet requested
 * Run THIS function to switch to 1aLKEEw7Nx0O1DbZjnXnhOeDjcLRloLN0Y8K2SscZKIc
 */
function switchToNewSheet() {
  updateSpreadsheetId('1aLKEEw7Nx0O1DbZjnXnhOeDjcLRloLN0Y8K2SscZKIc');
}

/**
 * Restore configuration using existing saved credentials
 * Run this if you see "No configuration found" but your credentials are set
 */
function restoreConfigurationFromProperties() {
  console.log('Restoring configuration from existing properties...');
  const props = PropertiesService.getScriptProperties();
  const sid = props.getProperty('IMPACT_SID');
  const token = props.getProperty('IMPACT_TOKEN');
  const spreadsheetId = props.getProperty('IMPACT_SPREADSHEET_ID');
  ...
  if (!sid || !token) {
    console.error('❌ Credentials not found. Please run setupWithTestValues() instead.');
    return;
  }

  // Use production configuration by default for stability
  setupProductionConfiguration(sid, token, spreadsheetId);
